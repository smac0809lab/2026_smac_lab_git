...
class GpsImuStanley(Node):
    def __init__(self):
        ...
        self.steer_pid = PIDController(kp=9.7, ki=0.1, kd=0.5, dt=0.05)
    ...
    def init_csv_logger(self):
        ...
        header = "time,utm_x,utm_y,path_type,vehicle_yaw_deg,path_yaw_deg,cte_meter,k_gain,v_kmh,target_steer_deg,current_steer_deg,kp,ki,kd,closest_cluster_dist,r_state,y_state,g_state,f_state,l_state,light_area,stopline_dist\n"
        with open(self.log_path, 'w') as f: f.write(header)

    def save_to_csv(self, x, y, p_type, v_yaw, p_yaw, cte, k_gain, v_kmh, target_steer, curr_steer, p_gain, i_gain, d_gain, cluster_dist, r, y_flag, g, f, l, area, dist):
        t = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        line = (f"{t},{x:.4f},{y:.4f},{p_type},{np.rad2deg(v_yaw):.2f},{np.rad2deg(p_yaw):.2f},"
                f"{cte:.3f},{k_gain:.2f},{v_kmh:.2f},"
                f"{target_steer:.2f},{curr_steer:.2f},{p_gain:.2f},{i_gain:.2f},{d_gain:.2f},{cluster_dist:.2f},"
    ...
    def joy_cb(self, msg):
        with self.lock:
            ...
            if not self.is_autonomous:
                ...
                self.manual_steer = -msg.axes[2] * 255.0
    ...
    def main_control_loop(self):
        ...
        self.save_to_csv(curr_x, curr_y, self.path_type_str, curr_yaw, path_yaw, cte, self.k, curr_v * 3.6, 
                         self.target_steer_deg, curr_steer, 
                         self.steer_pid.kp, self.steer_pid.ki, self.steer_pid.kd, 
                         curr_closet_dist, 
