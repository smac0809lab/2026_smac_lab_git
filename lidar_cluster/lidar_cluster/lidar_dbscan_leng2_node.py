...
from std_msgs.msg import Float32  # 가장 가까운 거리를 발행하기 위해 추가
...
class LidarDBSCANNode(Node):
    ...
    + 가장 가까운 클러스터의 rmin 값을 별도 토픽으로 발행
    """
    ...
    def __init__(self):
        ...
        # [추가] 가장 가까운 거리를 발행할 토픽 퍼블리셔
        self.closest_dist_pub = self.create_publisher(Float32, '/closest_cluster_distance', 10)

        self.get_logger().info(f"Subscribed to: {scan_topic}")
        self.get_logger().info("Publishing: /lidar_dbscan_markers")
        self.get_logger().info("Publishing closest distance to: /closest_cluster_distance")
    ...
    def cb_scan(self, scan: LaserScan):
        ...
        for k, c in enumerate(clusters):
            ...
            # [추가] 현재 클러스터의 최소 거리가 전체 중 가장 가까운지 확인
            if rmin < min_distance_found:
                min_distance_found = rmin
            ...
            if show_line:
                ...
                line.scale.x = line_w  # 기존 하드코딩(0.03) 대신 파라미터 적용
        ...
        # [추가] 가장 가까운 거리 데이터 토픽 발행
        # 유효한 클러스터가 하나도 없으면 inf 상태이므로, 감지되었을 때만 발행합니다.
        if min_distance_found != float('inf'):
            dist_msg = Float32()
            dist_msg.data = min_distance_found
            self.closest_dist_pub.publish(dist_msg)
